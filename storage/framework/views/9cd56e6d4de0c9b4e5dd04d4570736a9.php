<aside 
    ...
    <div class="flex flex-col h-full">
        <div class="p-6 flex items-center justify-between border-b border-white/10">
            <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-3 group">
                <div class="bg-white p-1.5 rounded-full shadow-inner">
                    <img src="<?php echo e(asset('images/pizza-boxx-logo.png')); ?>" alt="Logo" class="h-8 w-8">
                </div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto custom-scrollbar">
            ...
            <a href="<?php echo e(route('pegawai.dashboard')); ?>"
               class="flex items-center gap-3 px-4 py-4 rounded-2xl font-bold transition-all duration-300
                      <?php echo e(request()->routeIs('pegawai.dashboard') ? 'bg-white text-brand-red shadow-xl' : 'text-white/80 hover:bg-white/10 hover:text-white'); ?>">
            ...
            <a href="<?php echo e(route('pegawai.orders.index')); ?>"
               class="flex items-center gap-3 px-4 py-4 rounded-2xl font-bold transition-all duration-300
                      <?php echo e(request()->routeIs('pegawai.orders.*') ? 'bg-white text-brand-red shadow-xl' : 'text-white/80 hover:bg-white/10 hover:text-white'); ?>">
            ...
            <a href="<?php echo e(route('pegawai.deliveries.index')); ?>"
               class="flex items-center gap-3 px-4 py-4 rounded-2xl font-bold transition-all duration-300
                      <?php echo e(request()->routeIs('pegawai.deliveries.*') ? 'bg-white text-brand-red shadow-xl' : 'text-white/80 hover:bg-white/10 hover:text-white'); ?>">
                <i class="fa-solid fa-truck-fast w-5"></i>
                <span>Pengantaran</span>
            </a>
            
            <?php if(Auth::guard('employee')->check() && Auth::guard('employee')->user()->hasRole('admin')): ?>
                ...
                <a href="<?php echo e(route('filament.admin.pages.dashboard')); ?>"
                   class="flex items-center gap-3 px-4 py-4 rounded-2xl font-bold transition-all duration-300
                          <?php echo e(request()->routeIs('filament.admin.pages.dashboard') ? 'bg-red-800 text-white shadow-inner' : 'text-white/80 hover:bg-white/10 hover:text-white'); ?>">
        ...
        <div class="p-4 border-t border-white/10">
            <form action="<?php echo e(route('pegawai.logout')); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <button type="submit" 
...
</style><?php /**PATH C:\xampp\htdocs\pizza-boxx-web-v2\resources\views/partials/employee/sidebar.blade.php ENDPATH**/ ?>