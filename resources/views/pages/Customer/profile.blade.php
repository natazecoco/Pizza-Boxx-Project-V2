            ...
            <aside class="lg:w-1/4 w-full">
                <div class="bg-white p-4 rounded-[2rem] sticky top-28 border-2 border-gray-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                    <div class="grid grid-cols-2 lg:grid-cols-1 gap-3">
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="lg:mt-4 col-span-2 lg:col-span-1">
                            @csrf
                    ...
                    <div class="bg-white rounded-[2.5rem] p-8 lg:p-10 border-2 border-gray-900 shadow-sm">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            @forelse($addresses as $address)
                                <div class="relative group p-6 rounded-2xl border-2 transition-all duration-300 {{ $address->is_primary ? 'border-brand-red bg-red-50/30' : 'border-gray-200 bg-white hover:border-gray-400' }}">
                                    ...
                                    <div class="flex flex-col md:flex-row justify-between gap-6">
                                        <div class="flex-grow">
                                            <div class="flex items-center gap-3 mb-2">
                                                <span class="px-2 py-1 bg-gray-900 text-white text-[9px] font-black uppercase rounded-md tracking-widest">{{ $address->label }}</span>
                                                {{-- Menampilkan Nama Penerima --}}
                                                <span class="text-xs font-bold text-gray-500">{{ $address->receiver_name ?? $address->phone }}</span>
                                            </div>
                                            
                                            {{-- LOGIC TAMPILAN: Jika ada detail_address, tampilkan. Jika tidak, pakai legacy address --}}
                                            <h4 class="text-sm font-black text-gray-900 uppercase leading-tight mb-1">
                                                {{ $address->detail_address ?? Str::limit($address->address, 50) }}
                                            </h4>
                                            
                                            {{-- Tampilkan Map Address jika ada --}}
                                            @if($address->map_address)
                                                <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wide leading-relaxed mt-1">
                                                    <i class="fas fa-map-pin mr-1 text-brand-red"></i> {{ Str::limit($address->map_address, 80) }}
                                                </p>
                                            @endif
                                        </div>
                                        <div class="flex flex-row md:flex-col gap-2 shrink-0">
                                            {{-- Pass data JSON ke modal --}}
                                            <button onclick="openAddressModal({{ json_encode($address) }})" class="bg-white border-2 border-gray-200 text-gray-600 w-10 h-10 rounded-xl flex items-center justify-center hover:border-gray-900 hover:text-gray-900 transition-all shadow-sm"><i class="fas fa-pen text-xs"></i></button>
                                            @if(!$address->is_primary)
                                                ...
                                                <form action="{{ route('user.address.set-primary', $address->id) }}" method="POST">
                                                    @csrf
...
{{-- --- 4. MODAL FORM ALAMAT (Updated Fields) --- --}}
<div id="addressModal" class="fixed inset-0 z-[100] hidden bg-gray-900/80 backdrop-blur-sm p-4 flex items-center justify-center">
    <div class="bg-white w-full max-w-4xl rounded-[2.5rem] border-2 border-gray-900 shadow-2xl overflow-hidden animate-pop-in max-h-[90vh] flex flex-col">
        ...
        <form id="addressForm" action="{{ route('user.address.store') }}" method="POST" class="overflow-y-auto custom-scrollbar p-6 lg:p-8">
...
<script>
    ...
    function openAddressModal(data = null) {
        ...
        if (data) {
            ...
            let updateRoute = "{{ route('user.address.update', ':id') }}";
            form.action = updateRoute.replace(':id', data.id);
            methodInput.value = "PUT";
            submitBtn.innerText = "Simpan Perubahan";
            
            // Populate Fields
            document.getElementById('in-receiver').value = data.receiver_name || "{{ Auth::user()->name }}";
            ...
            document.getElementById('in-receiver').value = "{{ Auth::user()->name }}";
        }
