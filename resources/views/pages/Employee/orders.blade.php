...
<!-- Header Halaman -->
        ...
        <div class="bg-yellow-50 p-6 rounded-lg shadow-md flex items-center justify-between">
            <div>
                ...
                <p class="text-4xl font-bold text-yellow-600 mt-2">
                    {{ $orders->whereIn('status', ['accepted', 'preparing', 'ready_for_delivery', 'ready_for_pickup', 'on_delivery'])->count() }}
                </p>
            </div>
    ...
    <div class="mt-8" id="orders-container">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-white p-4 rounded-lg shadow-md border-t-4 border-yellow-500">
                ...
                <div class="space-y-4">
                    @foreach($orders->whereIn('status', ['accepted', 'preparing', 'ready_for_delivery', 'ready_for_pickup', 'on_delivery']) as $order)
                        <div x-show="filter === 'all' || filter === '{{ $order->order_type }}'" 
                            x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 transform scale-95">
                            @include('partials.employee.order-card', ['order' => $order])
            ...
            <div class="bg-white p-4 rounded-lg shadow-md border-t-4 border-green-500">
                ...
                <div class="space-y-4">
                    @foreach($orders->whereIn('status', ['completed', 'delivered']) ->sortByDesc('updated_at') as $order)
                        <div x-show="filter === 'all' || filter === '{{ $order->order_type }}'" 
                             x-transition:enter="transition ease-out duration-300"
                             x-transition:enter-start="opacity-0 transform scale-95">
                            @include('partials.employee.order-card', ['order' => $order])
    ...
    <div x-show="showPinModal" 
        ...
        <div class="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden transform transition-all"
            ...
            <div class="p-8">
                {{-- Display PIN yang Sedang Diketik --}}
                <div class="mb-6">
...
<script>
    ...
    // 1. FUNGSI UPDATE SEMUA TIMER (Single Loop)
    function updateAllTimers() {
        ...
        timers.forEach(badge => {
            ...
            // OPTIMASI B: Jika sudah selesai, jangan dihitung lagi
            ...
            // OPTIMASI C: Gunakan Date.now() untuk performa lebih cepat
            ...
            // OPTIMASI A: Logika warna yang efisien
            if (diff >= sla) {
                // Hanya ganti jika belum merah
                if (!badge.classList.contains('bg-red-600')) {
                    badge.className = "sla-timer-badge px-2 py-0.5 rounded font-bold uppercase tracking-wider flex items-center gap-1 bg-red-600 text-white animate-pulse transition-all duration-500";
                }
            } else if (diff >= (sla * 0.7)) {
                // Hanya ganti jika belum kuning
    ...
    function startGlobalTimers() {
        if (globalTimerInterval) return; // Mencegah interval ganda
        updateAllTimers(); // Jalankan sekali di awal
        globalTimerInterval = setInterval(updateAllTimers, 1000); // Detak per detik
    }
    ...
    function refreshOrders() {
        ...
        .then(html => {
            ...
            // Update isi grid tanpa refresh halaman
            ...
            // NOTE: Kita tidak perlu panggil startGlobalTimers() lagi di sini
            // karena interval global sudah jalan terus di latar belakang.
        })
    ...
    function submitPin(orderId, pinCode) {
        ...
        .then(data => {
            if (data.success) {
                ...
                refreshOrders(); // Langsung update tampilan
            } else {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        startGlobalTimers(); // Hidupkan stopwatch global
        setInterval(refreshOrders, 15000); // Cek pesanan baru tiap 15 detik
    });
</script>
@endpush