...
@section('title', 'Verifikasi Pengambilan #' . $order->id)
...
<div class="container mx-auto max-w-lg py-12 px-4">
    ...
    {{-- --- 1. NAVIGASI KEMBALI --- --}}
    <div class="mb-6">
        <a href="{{ route('pegawai.dashboard') }}" class="inline-flex items-center text-xs font-black uppercase tracking-widest text-gray-400 hover:text-brand-red transition-all group">
            <i class="fas fa-arrow-left mr-2 transform group-hover:-translate-x-1 transition-transform"></i>
            Kembali ke Dashboard
        </a>
    </div>

    {{-- --- 2. KARTU RINGKASAN PESANAN (Konteks Pegawai) --- --}}
    <div class="bg-gray-900 rounded-t-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden border-b border-white/10">
        {{-- Pizza Pattern Subtle --}}
        ...
        <div class="relative z-10">
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <h2 class="text-2xl font-black italic uppercase tracking-tighter">#{{ $order->id }} - {{ $order->customer_name }}</h2>
                </div>
            ...
            <div class="flex gap-4 text-[10px] font-black uppercase tracking-widest text-white/50">
                <span class="flex items-center gap-1.5"><i class="fas fa-pizza-slice text-brand-kraft"></i> {{ $order->orderItems->sum('quantity') }} Items</span>
                <span class="flex items-center gap-1.5"><i class="fas fa-wallet text-brand-kraft"></i> Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
